...
**Reels parity (added May 2026).** The same keyword surface applies to IG Reels — the topic classifier reads the Reels caption identically to feed posts. Reels carry a slightly tighter `13–18` keyword count than carousels (Reels captions are scrolled past faster; longer keyword blocks dilute the engagement signal). Reels tooling lives in `scripts/lib/render-platform-caption.mjs`; the LLM emits the array under `caption.instagramKeywords` and the scrubber rejects count violations, hashtag duplicates, and banned terms. Per-reel audience-angle notes belong in `docs/plans/IG-KEYWORDS.md` alongside carousel rationale; final keyword arrays live in the saved plan JSON at `output/plans/<level>/<slug>.json`.

**TikTok equivalent (added May 2026).** TikTok 2026 weights caption text + on-screen text + audio transcript with roughly equal weight; bracketed multilingual blocks read as spam there, and TikTok auto-translates so multilingual stuffing wastes characters. The TikTok-equivalent of the keyword block has two parts:

1. **`caption.tiktokTopics`** — 3–5 natural-language English search-query phrases, rendered as a single `Topics: phrase1, phrase2, phrase3` line above the hashtag wave. Phrases mirror how someone types into TikTok Search (which hit ~50% of Gen Z product searches by 2025). Stay grounded in the post's actual content; never byte-equal to an `instagramKeywords` entry — rephrase as a natural search query.
2. **`caption.tiktokExtraTags`** — 0–3 niche TikTok-native hashtags (`#moneytok`, `#financetok`, `#financialeducation`, `#expatlifehacks`, `#europemoney`) appended to the TikTok caption ONLY (IG enforces the 5-cap; TikTok tolerates 5–7). Banned: `#fyp`, `#foryou`, `#foryoupage` (saturated, flagged).
...
- **Blog posts** ship without memes but with a defined *figure* language — clean conceptual SVG diagrams that crystallize one idea per figure, designed to avoid math-anxiety triggers for the Eva-type primary audience. Full figure rules in `blog-content-plan.md` Editorial Rule 9; nine figures shipped across Building posts #17, #20, #23, #24, #25, #26, #28, #30 establish the visual grammar.
